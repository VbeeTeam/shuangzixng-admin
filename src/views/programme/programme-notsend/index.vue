<template>
    <div class="app-container notsend">
        <el-form>
            ...
            <el-form-item label="生日时间" label-width="110px">
                <div class="block">
                    ...
                    <ul>
                        <li v-for="(item1,index4) in totalTabcus" :key="index4">
                           ...
                           <div v-for="(item2,index7) in item1.planDetail" :key="index7">
                                <span v-if="item2.insureLines">{{item2.insureType}}保障：{{item2.insureLines}}W</span>
        ...
        <el-table class="tableStyle" border ref="multipleTable" :data="notcusList" :header-cell-style="tableHeaderColor"
            ...
            <el-table-column label="方案" width="100">
                <template slot-scope="scope">
                    <span class="copy-btn" style="color:blue;border-bottom:1px solid blue;" @click="doCopy(scope.row.customer.userId)">复制链接</span>
        ...
        <div class="table-btn">
            ...
            <el-button size='small' type="info" @click="toSendDialog()">发送</el-button>
        ...
        <div class="dialogSend">
            <el-dialog :visible.sync="dialogSend" :modal-append-to-body='false' top="50vh">
                <el-form>
                    <el-form-item label="发送方式">
                        <el-radio v-model="send.radioSend" label="0">立即发送</el-radio>
                        <el-radio :disabled="send.disabled" v-model="send.radioSend" label="1">定时发送</el-radio>
                    </el-form-item>
                    <el-form-item label="日期" label-width="100px" v-if="send.radioSend == 1">
                        <div class="block">
                            <el-date-picker v-model="send.date" type="datetime"
                    ...
                    <p v-if="send.sendToast && send.radioSend == 1">定时时间范围：<br/>  {{send.sendToast.timestamp}}到{{send.sendToast.message}}</p>
                </el-form>
                <div class="btn">
                    <el-button type="success" size="small" round @click="toSend()">发送</el-button>
                    <el-button type="info" size="small" round @click="cancelSend()">取消</el-button>
                </div>
            </el-dialog>
    ...
    methods: {
        ...
        toSendDialog() {
            if (this.cus.length == 1) {
                initData('/api/customerOrder/getAvailableSessionDateTime/' + this.cus[0].customer.unionId).then(res => {
                    if (res.status == 101) {
                        this.toast('该用户未创建会话，无法定时发送！')
                        this.send.disabled = true
                    } else {
                        this.send.radioSend = '1'
                        this.send.disabled = false
                        this.send.sendToast = res
                        this.send.date = this.cus[0].sendingTime
                    }
                })
        ...
        toSend() {
            ...
            if (!this.send.date && this.send.radioSend == 1) {
                this.toast('请选择日期')
            } else {
                var obj2 = {
                    id: this.selectId,
                    sendingTime: this.send.radioSend == 1 ? this.send.date : '',
                    type: this.send.radioSend
                }
                postData('/api/customerOrder/send', obj2).then(res => {
