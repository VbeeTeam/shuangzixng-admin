    ...
    <div class="app-container">
        <div class="head-container topInfo">
            <el-form :inline="true" class="demo-form-inline">
                <el-form-item :label="item.type" v-for="(item, index) in status" :key="index">
        ...
        <el-table :data="list" v-loading.body="listLoading" :header-cell-style="tableHeaderColor" element-loading-text="Loading" border fit highlight-current-row>
            ...
            <el-table-column label="操作(点击切换上下架)" align="center" width="250">
                <template slot-scope="scope">
                    <el-button class="" type="text" size="mini" @click.native="edit(scope.row.id)">修改</el-button>
                    ...
                    <el-button :class="scope.row.selling ? 'btn-success' : 'btn-warning'" type="text" size="mini" @click.native="setSelling(scope.row, scope.$index)">{{ scope.row.selling ? '已上架' : '已下架' }}</el-button>
...
<script>
import { getProductList, getCompany, delProduct, sellingProduct, statList } from '@/api/product'
